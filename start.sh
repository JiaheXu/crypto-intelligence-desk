...
TELEGRAM_FINANCE_INCOMING_LOG_DIR=${TELEGRAM_FINANCE_INCOMING_LOG_DIR:-"$APP_DIR/../state/telegram_finance_incoming"}
export TELEGRAM_FINANCE_INCOMING_LOG_DIR
TELEGRAM_FINANCE_DEDUPE_SIMILARITY_THRESHOLD=${TELEGRAM_FINANCE_DEDUPE_SIMILARITY_THRESHOLD:-0.90}
TELEGRAM_FINANCE_DEDUPE_WINDOW_SECONDS=${TELEGRAM_FINANCE_DEDUPE_WINDOW_SECONDS:-14400}
TELEGRAM_FINANCE_DEDUPE_RECENT_LIMIT=${TELEGRAM_FINANCE_DEDUPE_RECENT_LIMIT:-200}
export TELEGRAM_FINANCE_DEDUPE_SIMILARITY_THRESHOLD TELEGRAM_FINANCE_DEDUPE_WINDOW_SECONDS TELEGRAM_FINANCE_DEDUPE_RECENT_LIMIT
...
case "$INFO_PARSER_BACKEND" in
  ...
  local)
    ...
    INFO_PARSER_MODEL=${INFO_PARSER_MODEL:-${OPENAI_MODEL:-gpt-5.3-codex-spark}}
    ;;
  *)
...
mkdir -p "$TELEGRAM_FINANCE_INCOMING_LOG_DIR"
...
printf '%s\n' "Telegram finance incoming log: $TELEGRAM_FINANCE_INCOMING_LOG_DIR"
printf '%s\n' "Telegram finance UDP: $FINANCE_NEWS_PROXY_UDP_HOST:$FINANCE_NEWS_PROXY_UDP_PORT"
printf '%s\n' "Telegram finance dedupe: similarity>=$TELEGRAM_FINANCE_DEDUPE_SIMILARITY_THRESHOLD window=${TELEGRAM_FINANCE_DEDUPE_WINDOW_SECONDS}s recent=$TELEGRAM_FINANCE_DEDUPE_RECENT_LIMIT"
